...
# Custom modules
from yfinance_constants import (
    YFINANCE_TEST_TICKER,
...
def test_invalid_ticker_error(mock_invalid_ticker_error):
    ...
    with pytest.raises(ValueError):
        manager.set_ticker(YFINANCE_TEST_TICKER)
...
def test_socket_failure(mock_closed_connection):
    ...
    with pytest.raises(ConnectionClosed):
        manager.set_ticker(YFINANCE_TEST_TICKER) # Ticker is required before socket starts.
        manager.run_socket(mock_callable)
...
def test_yfinance_successful_run_socket(mock_yfinance_successful_run_socket):
    ...
    manager.set_ticker(YFINANCE_TEST_TICKER)
    manager.run_socket(mock_callable)
    manager.socket.subscribe.assert_called_once_with(YFINANCE_TEST_TICKER)
