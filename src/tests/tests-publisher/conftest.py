...
def publisher_factory_fixture(mocker):
    mocker.patch(MOCK_PATHS["set_publisher_client"], return_value=mocker.MagicMock())
...
def google_api_error_patch_returner(mocker, path):
    return mocker.patch(
        path,
        side_effect=GoogleAPICallError(
            "Bad Request",
        ),
    )
...
@pytest.fixture
def mock_google_api_error(mocker, publisher_factory_fixture):
...
@pytest.fixture
def mock_pub_sub_topic_path(mocker, publisher_factory_fixture):
...
@pytest.fixture
def mock_publish_message(mocker, publisher_factory_fixture):
    mocker.patch(MOCK_PATHS["publish_message"], return_value=mocker.MagicMock())
    return publisher_factory_fixture
